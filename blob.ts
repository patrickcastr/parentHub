...
export async function uploadFile(containerName: string, blobName: string, content: Buffer, contentType: string): Promise<void> {
...
export function getSignedUrl(containerName: string, blobName: string, expirySeconds: number): string {
  ...
  // The account key lives on the underlying credential
  const key = (service.credential as any).accountKey;
