...
export async function createCohort(name: string, expiresAt?: string, teacher?: string): Promise<CohortEntity> {
  const id = uuidv4();
...
export async function listCohorts(): Promise<CohortEntity[]> {
  return queryEntities<CohortEntity>(TABLE, `PartitionKey eq 'cohort' and (archived eq false or archived eq null)`);
}
