...
export default function AdminPage() {
  ...
  async function importCsv(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    // For CSV import we require the cohortId to be passed via query param; here we pick the first cohort
  ...
  return (
    <div>
      ...
      <ul>
        {cohorts.map(c => (
          <li key={c.id}>{c.name} {c.expiresAt && `(expires ${c.expiresAt})`}</li>
        ))}
      </ul>
      <h2>Students</h2>
