...
// Legacy files router removed (archived). New Azure-based router lives under src/server/routes.
...
if (!IS_PROD && BYPASS_AUTH) {
  ...
  app.get('/api/me', (_req, res) => {
    // Legacy endpoint (dev bypass). Prefer /api/auth/me in new code.
    res.json({
      id: 'dev-user',
...
// NOTE: Public endpoints (health, groups list, students CRUD for now) remain open in MVP.
...
// Mount groups router at /api so its internal /groups paths resolve to /api/groups
app.use('/api', groups);
...
// Mount auth router BEFORE protected routes so cookies are parsed and sessions issued
app.use('/api/auth', authRouter);
if (process.env.NODE_ENV !== 'production') console.log('[AUTH] router mounted at /api/auth');
// Temporary debug route for session introspection (DO NOT enable in production long-term)
...
if (process.env.DATABASE_URL){
  try { const u = new URL(process.env.DATABASE_URL); console.log(`[db] host=${u.hostname} db=${u.pathname.slice(1)} provider=postgresql`); } catch {}
}

log.info({ account: process.env.AZURE_STORAGE_ACCOUNT, container: process.env.AZURE_STORAGE_CONTAINER }, '[storage] config');

// GET /api/me (protected when not bypassing)
if (IS_PROD || !BYPASS_AUTH) app.get('/api/me', requireAuth, async (req: any, res) => {
  ...
  res.json({ name, email, role, student: student ? { id: student.id, groupId: student.groupId, groupName } : null });
});
